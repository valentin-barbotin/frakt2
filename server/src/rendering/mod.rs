...
use shared::structs::range::Range;
...
pub fn launch_graphics_engine(
    server: Arc<Mutex<Server>>,
    rendering_data_receiver: Receiver<RenderingData>,
) -> Result<(), Error> {
    ...
    thread::spawn(move || loop {
        while let Ok(data) = rendering_data_receiver.recv() {
